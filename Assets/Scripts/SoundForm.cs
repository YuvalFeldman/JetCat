...
public class SoundForm : MonoBehaviour {
	
	//if true a menu will apear ingame with all the microphones
	[HideInInspector]
		bool SelectIngame = false;
	...
	float amountSamples=256;
	static float loudness;
	float sensitivity=0.4f;
	float sourceVolume=100;
	private int minFreq;
	int maxFreq=44100;
	...
	void  Update (){
		//transform.localScale = new Vector3 (1f, loudness * 20f, 1f);
		/*
		if (loudness * 20f >= 2) {
			//transform.localScale = new Vector3 (1f, loudness * 20f, 1f);
			transform.localScale = new Vector3 (1f, 5, 1f);
		} else {
	...
	public float GetDataStream (){
		if(Microphone.IsRecording(selectedDevice)){
			...
			float[] dataStream  = new float[(int)amountSamples];
			float audioValue = 0;
			GetComponent<AudioSource>().GetOutputData(dataStream,0);
			
			foreach(var i in dataStream){
